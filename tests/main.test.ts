...
describe('SyrinscapePlugin', () => {
  ...
  describe('loadExternalScript (via prototype access)', () => {
    it('resolves when script loads successfully', async () => {
      ...
      vi.spyOn(document.head, 'appendChild').mockImplementation((el) => el as HTMLElement);

      const loadScript = (plugin as unknown as { loadExternalScript: (url: string) => Promise<void> }).loadExternalScript.bind(plugin);
    ...
    it('rejects when script fails to load', async () => {
      ...
      const loadScript = (plugin as unknown as { loadExternalScript: (url: string) => Promise<void> }).loadExternalScript.bind(plugin);
      await expect(loadScript('https://example.com/bad.js')).rejects.toThrow('Failed to load script');
      vi.restoreAllMocks();
    });
  });

  describe('loadSyrinscapeScripts (via prototype access)', () => {
    it('loads three scripts', async () => {
      const appendSpy = vi.spyOn(document.head, 'appendChild').mockImplementation((el) => {
        const script = el as HTMLScriptElement;
        setTimeout(() => { if (script.onload) script.onload(new Event('load')); }, 0);
        return el as HTMLElement;
      });

      const loadScripts = (plugin as unknown as { loadSyrinscapeScripts: () => Promise<void> }).loadSyrinscapeScripts.bind(plugin);
      await loadScripts();
